...
| 86 | **系统级安全/事务完整性审查：修复 5 高危 + 7 中危（v1.1.6）**（FR-CMP-031/032/033、FR-CFG-002/003/004、FR-SEC-003/007、FR-API-001、FR-NET-017、FR-SYS-004/010） | 起因：对约 120 个产品文件做一轮系统级审查（两个 explore 子代理 + 人工复核），产出 5 高危 / 约 14 中危 / 约 25 低危；本轮修复全部高危与 7 项中危，**均为不动契约的缺陷修复**（违反的恰是既有契约/决策，如 FR-SEC-007、决策 #70）。**高危**：① **镜像名路径穿越**——import/delete/download 接受 `../../etc/passwd` 类镜像名，可写/删仓库目录外任意路径；修以 `validateName` 按类型分流（VM 名 `^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$`；容器名允许 `:`——`alpine:3.20` 必须过——但拒绝 `/` 与前导点；`index.json` 等保留名拒收）。② **非持有者 commit 隐式取消他人回滚**——`commit confirmed` 等待期内任何其他会话 commit 一次（哪怕校验失败）就 AppendRevision 平掉回滚基线；修以隐式确认移到 requireHolderLocked 之后。③ **confirmed 超时回滚不下发**——只写库不 Apply，DB 说"已回滚"而底层仍运行变更后配置；修以回滚后补 Apply，失败记审计 + 告警（committed 以 DB 为准不回退修订）。④ **`\| display json/xml` 泄露敏感字段**——管道直接序列化 model 树，`password_hash` 原样输出，绕过 REST 侧脱敏（决策 #70）；修以 `redactedStructured`（深拷贝 + 脱敏，键名判定同源 `IsSensitiveKey`）。⑤ **sweep 清扫 stale 锁后持有者复活**——清扫不走 ReleaseLock 不记录 Holder，持有者下一次 Refresh 把锁刷回有效；修以记录 Holder 调 ReleaseLock。**中危**：bond Apply 锁外写 map（并发即 runtime fatal）+ DeleteBond 半状态（delete 分支回锁内、删除按「快照→detach→BondDelete→登记未重建才清理」）；VPP stats 坏连接永久缓存（错误路径 `statsReset`）；`ParseCoreList` 单 token OOM（展开前上界检查 1024 条）；**`commit and-quit` 失败语义**——失败也释放锁退出配置模式、candidate 去向错乱，修以**成功才 Release+回 oper，失败留配置模式、candidate 保留**并提示「校验失败（仍处于配置模式，candidate 保留）」（命令全表描述同步更正）；**登录面加固**——login 与 cli/execute 是仅有的两个不经 decodeBody 4MiB 限制的 JSON 端点，补 1MiB `MaxBytesReader`，用户不存在/未设口令路径执行 dummy PBKDF2 抹平时间差（防用户名枚举），`http.Server` 补 ReadTimeout/IdleTimeout=120s（**不设 WriteTimeout——会杀 `/events` SSE**）；**syslog 持锁拨号**——目标不可达时 3s 拨号在互斥锁内拖死全部日志转发，修以拨号出锁 + 失败 30s 熔断（Configure 换目标重置）+ 写仅持锁期间加 5s deadline（TCP octet-counting 帧需串行；写失败保持「下条重连」语义）。**附带**：`check_openapi_json_sync.sh` 在 Windows GBK 控制台打印 `⇄` 触发 UnicodeEncodeError → make check 误报「未同步」，reconfigure UTF-8。**守护**：12 项修复各附红-绿测试（TestEngineCommitByNonHolderDoesNotConfirmPending、TestEngineCommitConfirmedTimeout、TestPipeDisplayRedactsSensitiveFields、TestEngineSweepReleasesStaleLockAfterRestart、TestBondConcurrentApplyDelete、TestUnboundedBodyEndpointsRejected、TestDummyHashTimingEqualizer、TestForwardDialFailureCooldown 等）。**真机复验**（v1.1.6-dev 部署 /tmp）：fulltest 197/1/2（唯一失败为 #34 既有占位 `show vpp runtime`，非回归）、语义校验 11/0、pty 冒烟 10/10、and-quit 定向 pty 验证 6/6（成功回 oper 且锁释放；失败留配置模式、`exit` 被「存在未提交变更」拦下、丢弃后才释放锁）。**发布**：PR #90 → v1.1.6（deb 由 nfvis-vm `make deb` 生成，sha256 88368ccb…；装机后 `show version` 报 NFViS 1.1.6）。**登记未修（涉契约语义，待先改契约再动代码）**：配置模式 `run request` 绕过 login class 授权、backup/tech-support 下载端点 ClassReadOnly→应 ClassSuperUser、VRF 静态路由 PUT 只增不删、ACL 解绑语句缺失。证据 `docs/evidence/v1-closeout-round16.txt` |
