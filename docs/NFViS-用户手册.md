...
### 10.12 Web 控制台（总览 / 配置 / 诊断）

浏览器打开 **`https://<管理地址>/api/v1/ui/`**（少打末尾斜杠会自动补上）。界面随 nfvisd 一起发布，
**不需要额外部署任何东西**，也不连外网。

**能看什么**：系统与组件版本、大页/隔离核资源池、数据面状态、接口（管理/链路/速率/驱动）、
虚拟机与容器、未解决告警、最近事件。页面每 5 秒兜底刷新一次，有事件推送时立即刷新。
组件版本取自本机探测：Ubuntu/VPP/libvirt/QEMU/Docker 探测不到时显示「—」，
DPDK 没有独立版本来源（随 VPP 一起编译），同样显示「—」。

**能改什么（配置卡）**：卡上先显示当前生效配置的版本号与内容预览，点「开始编辑」会以当前生效配置为
