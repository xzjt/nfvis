...
![登录页](images/console/01-login.jpg)

*图 10.12-1　登录页：用户名 + 口令，凭据只用于换取访问令牌。*

![总览页](images/console/02-overview.jpg)
...
![虚拟机列表](images/console/03-vms.jpg)

*图 10.12-3　虚拟机列表：状态与「操作」列按当前状态启用按钮（此图 vnf-b 处于 shutoff，故只有「启动」可点）。*

![虚拟机详情](images/console/04-vm-detail.jpg)

*图 10.12-4　虚拟机详情：资源/生命周期/界面口计数，四个页签在标题下方。*
...
![低危确认框](images/console/05-vm-start-confirm.jpg)

*图 10.12-5　低危档：列出这条动作做什么 + 只读回显对应命令行，点「确定」执行。*

![高危闸门](images/console/19-user-create-gate.jpg)

*图 10.12-6　高危档（此处为「创建本地用户」）：逐条列出影响面、**要求手工输入确认词**（区分大小写，
输入不匹配时「执行」保持不可点），并**等 10 秒倒计时走完**；倒计时期间可随时取消。*
...
**各域页面一览**（图为真机实拍，可对照本机同名页面）：

![系统页](images/console/12-system.jpg)

*图 10.12-7　系统域：状态、组件版本、以及「已生效系统配置段」（口令哈希等敏感叶子不回显）。*

![网络对象](images/console/08-network.jpg)

*图 10.12-8　网络对象：VRF / ACL / NAT / 聚合 / QoS / 端口镜像六类一张页；大表按需拉取。*

![虚拟交换机](images/console/09-switches.jpg)

*图 10.12-9　虚拟交换机：类型、成员端口、数据面 BD 号，以及成员口的实时收发包计数。*

![虚拟交换机详情](images/console/10-switch-detail.jpg)

*图 10.12-10　虚拟交换机详情：成员口管理/链路状态与 MAC 学习表（MAC 表按需拉取，避免大表拖慢页面）。*

![LLDP](images/console/11-lldp.jpg)

*图 10.12-11　LLDP：全局开关与接口条目（属配置，提交后生效）、邻居表（运行态；无对端时为空属正常）。*

![硬件健康](images/console/13-hardware.jpg)

*图 10.12-12　硬件健康：带外管理/传感器/磁盘 SMART 与健康阈值。采不到的项如实显示「—」，不编造。*
...
*图 10.12-14　接口：链路/速率与收发计数。*

![接口详情](images/console/16-iface-detail.jpg)

*图 10.12-15　接口详情：配置与运行态对照，下方是驱动接管（DPDK 绑定/解绑）与 SR-IOV 面板——
管理口由服务端守卫拒绝，界面不提供强制绕过入口。*

![内核基线](images/console/17-kernel.jpg)

*图 10.12-16　内核基线：运行实际 / 配置期望 / 差异三方对照（此机差异项为已登记的「大页 1G 期望 2 实际 3」）。*

![镜像](images/console/07-images.jpg)

*图 10.12-17　镜像仓库：类型、大小、引用数、导入状态；折叠区可经 URL 拉取 / incoming 文件 / 本地上传导入。*

![容器](images/console/06-containers.jpg)

*图 10.12-18　容器：本机未声明容器时如实显示「（无）」。*

**时间口径**：控制台里所有**记录类**绝对时间——审计、告警、事件、提交历史、编辑锁会话、
镜像与虚拟机创建、归档与抓包时间、证书有效期——一律按 **UTC** 显示并带 `UTC` 标记，
形状为 `2026-09-25 01:24:22 UTC`。这与 CLI（`show log audit`、`show configuration history` 等）
以及 `journalctl` 是同一口径，方便互相对照；服务器本机时区是 UTC 还是别的时区，都不改变这里的显示。
唯一例外是审计卡右侧的「（已刷新 时刻）」：它只是界面提示（说明这张卡片刚被你刷新过），
用的是浏览器的本地时间，不是记录时间。
...
![配置页](images/console/22-config.jpg)

*图 10.12-19　配置页：先显示当前生效版本（此机为 revision 57）与内容预览，点「开始编辑」建立候选。*
...
![提交历史](images/console/23-history.jpg)

*图 10.12-20　提交历史：每版可「看差异」或「回滚到这一版」（取为候选，两段式的第一段）。*

![编辑锁会话](images/console/24-sessions.jpg)

*图 10.12-21　编辑锁会话：只显示谁正持有候选配置的编辑锁，便于排查「改配置时提示锁被占用」。*
界面不提供强制夺锁；释放请在「配置」页「丢弃并结束编辑」。*
...
![用户与权限](images/console/18-users.jpg)

*图 10.12-22　用户与权限：用户表（改权限类 / 重置口令 / 删除）、自定义权限类、新建用户、以及右下角“修改我的口令”。*

![删除用户后的回显](images/console/20-user-deleted.jpg)

*图 10.12-23　写操作完成后页面回显结果（如「已删除用户 web-probe。」），并**立即交还编辑锁**——
同一时刻命令行也能马上进配置模式，不必等你登出。*
...
![证书页](images/console/21-tls.jpg)

*图 10.12-24　证书页：当前证书信息（有效期按 UTC 显示，便于与 `openssl x509 -noout -dates` 对照），
下方为安装外部证书与重签自签证书。*
...
![诊断页](images/console/25-diagnostics.jpg)

*图 10.12-25　诊断：经数据面的连通性测试与接口计数清零（管理口在内核平面，请在宿主机上 ping）。*

![抓包页](images/console/27-capture.jpg)

*图 10.12-26　抓包：开始（环形缓冲，超出深度丢最旧）、停止（丢弃）、停止并导出 pcap；导出件在同页下载。*

![运维动作页](images/console/26-actions.jpg)

*图 10.12-27　运维动作：备份/SSH host key/时间同步/诊断归档/core dump/清除已恢复告警，
右侧「归档下载」列出配置备份与诊断归档，「软件版本」一栏是升级与回退。*

![审计日志页](images/console/28-audit.jpg)

*图 10.12-28　审计日志：时间按 UTC 显示（与 `show log audit` 逐字一致），高危动作会留「意图 + 结果」两条。*
