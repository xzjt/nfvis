...
网络（网络对象、虚拟交换机）/ 系统（资源池、接口）/ 配置 / 运维（动作、审计、诊断、抓包）。
页面右上角有面包屑（如「计算 › 虚拟机」）。

**点对象看详情**：列表里的行可以直接点进**详情页**（虚拟机、容器、镜像、网络对象各有独立页面），
地址形如 `#/compute/vms/vnf-a`——可分享、可刷新保持。虚拟机详情页分「概览 / 接口 / 快照 / 串口」四个页签，
容器详情页分「概览 / 日志」；**离开详情页会自动断开串口连接**。面包屑末段可点，回到对应列表页。
...
有事件推送时立即刷新；**配置页与运维动作页不自动刷新**（免得你正在填表时被刷掉）。
每页只请求它自己需要的数据，不再整页全量刷新。**破坏性动作（重启数据面/重启主机/关机/重签证书）
只在「运维」页，不在总览页**，且都按影响面分级确认：低危点一次即可；中危先逐条列出影响面、
主按钮标红；高危还要手工输入确认词、等倒计时走完才能点「执行」。每个确认框里都只读回显这条
动作对应的命令行语句（便于工单对照，不需要你输入语句）。

**能看什么**：总览页给系统与组件版本、数据面状态、未解决告警与最近事件；计算/网络/系统各页给
虚拟机与容器（含 vhost-user 口计数）、镜像仓库、虚拟交换机（含成员口计数）、网络对象（VRF/ACL/
NAT/聚合/LLDP/QoS/端口镜像）、资源池与接口（管理/链路/速率/驱动）。
...
（谁持有可在 CLI 用 `show system configuration sessions` 看，见 §11.1）。之后的顺序是：

1. 在表单里改（系统 / 接口 / 资源池三节），或直接改原始 JSON；
...
**诊断卡**：经数据面执行 `ping`（填目标与次数；**未通即判失败**，失败时把底层原始回显一并显示出来），
以及「刷新服务端日志（末 100 行）」按需拉取服务端日志（日志不进 5 秒轮询，避免每次刷新都读盘）。

**界面上还没有的**：交互式 console 终端、SSH 主机密钥重签、core dump 导出、`load merge`、
`traceroute` 与接口统计清零，以及虚拟交换机/虚拟机详情里的统计字段——这些仍在 CLI（§4）
或 REST（§3.4）里做。
