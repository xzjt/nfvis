...
- 当前阶段：**M1、M2 已完成**（M2 DoD 现场演示记录见 `docs/M2-DoD-现场演示记录.md`）；当前 **M3 网络编排（govpp）**：先清 T0 尾巴（T0-1~T0-3），真机项受前置条件 M3-P0（nfvis-vm 初始化 + 手动 `systemctl start vpp`）阻塞。详见 `docs/M3-任务清单.md`。设计基线在 `docs/`，**不要凭记忆重设计**。
- 已定决策 29 项见规格书附录 A——实现中遇到"该怎么做"的问题，先查附录 A，不要重新发明。
