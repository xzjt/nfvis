...
- 验证环境 nfvis-vm 当前状态（**2026-09-18 由用户还原为全新 Ubuntu Server 26.04 后重建**）：
  ...
  nfvis **1.1.14** 由本机源码构建后 `dpkg -i` 安装（**尚未发布**；做法是「每次合并后按合并后的 main 重新构建装上」，故版本号随轮次增长——以 `nfvisd -version` 为准，含决策 #99~#102 与第一批发现的修复），
  ...
  **这台的版本矩阵比产品验证过的基线新**（内核 7.0 / libvirt 12 / qemu 10.2 / docker 29 / VPP 26.06-rc2），
  ...
  设计基线在 `docs/`，**不要凭记忆重设计**。
- 已定决策 102 项见规格书附录 A——实现中遇到"该怎么做"的问题，先查附录 A，不要重新发明。
- **V1 验收收口**：`docs/V1-验收检查表.md` 把规格书 **109 条 FR** 逐条对照证据
  ...
  **待办与未完成项的唯一入口见 `docs/V1-收尾待办.md`**（含环境要点与踩坑记录）。
  已发布 **v1.0.0 / v1.1.0 / v1.1.1 / v1.1.2 / v1.1.3 / v1.1.4 / v1.1.5 / v1.1.7 / v1.1.8 / v1.1.9 / v1.1.10**（见 GitHub Releases；
