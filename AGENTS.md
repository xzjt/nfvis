...
- 验证环境 nfvis-vm 当前状态（**2026-09-19 两度被恢复为干净快照**；快照恢复会清掉全部现场，
  ...
  **round81 后现状**（2026-09-26：决策 #155 `show` 族 JunOS 化第一步——接口族全运行态 + `show configuration | display set`
  ...
  nfvis **1.1.39** active（**已发布 v1.1.39**＝决策 #159 首装 AppArmor/快照 raw 排除 + #160 容器镜像命名闭环；v1.1.38＝#158 delete 反向写入、v1.1.37＝#157 多字节输入、v1.1.36＝#156 console TLS、v1.1.35＝#155 JunOS 化第一批；均回下载 sha256 一致）；
  ...
  **`nodejs` v22.22.1**（round37 装，
  ...
  设计基线在 `docs/`，**不要凭记忆重设计**。
- 已定决策 160 项见规格书附录 A——实现中遇到"该怎么做"的问题，先查附录 A，不要重新发明。
