...
- 当前阶段：**M1、M2 已完成**；**M3 进行中**：T0-1~T0-3、M3-P0、M3-1~M3-7 已合并（govpp 连接/startup.conf/L2/L3+BVI/ACL/NAT/SPAN/QoS/bond/LLDP/运行态端点），待办 **M3-8 恢复收敛、M3-9 CLI 操作命令、M3-10 集成测试与真机验收**。**新会话请先读 `docs/M3-进度交接.md`**（进度、环境与命令、踩坑、决策），任务清单见 `docs/M3-任务清单.md`。设计基线在 `docs/`，**不要凭记忆重设计**。
