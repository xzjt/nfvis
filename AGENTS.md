...
- 当前阶段：**M1 已完成；M2 的 W1~W9 任务已全部合并**（见 `docs/M2-收尾任务清单.md`），DoD 第 3 条「CLI 现场演示」待人工确认。下一步 M3（网络编排 govpp）。设计基线在 `docs/`，**不要凭记忆重设计**。
- 已定决策 28 项见规格书附录 A——实现中遇到"该怎么做"的问题，先查附录 A，不要重新发明。
