...
- 当前阶段：**M1、M2 已完成**；**M3 任务已全部交付**（T0-1~T0-3、M3-P0、M3-1~M3-10：govpp 连接/startup.conf/L2/L3+BVI/ACL/NAT/SPAN/QoS/bond/LLDP/运行态端点/恢复收敛+告警/CLI 操作命令/集成测试），下一步 **M4**（VNF/容器/镜像）。**新会话请先读 `docs/M3-进度交接.md`**（进度、环境与命令、踩坑、决策），M3 验收记录见 `docs/M3-10-集成测试验收记录.md`，任务清单见 `docs/M3-任务清单.md`。设计基线在 `docs/`，**不要凭记忆重设计**。
- 已定决策 34 项见规格书附录 A——实现中遇到"该怎么做"的问题，先查附录 A，不要重新发明。
