...
- 验证环境 nfvis-vm 当前状态（**2026-09-19 两度被恢复为干净快照**；快照恢复会清掉全部现场，
  ...
  **随快照清掉了**——跑 `make integration` 前需先重建（镜像需重导；布局与流程见待办 §3.3 / §0 第 1 条）。
  ...
  设计基线在 `docs/`，**不要凭记忆重设计**。
- 已定决策 112 项见规格书附录 A——实现中遇到"该怎么做"的问题，先查附录 A，不要重新发明。
- **V1 验收收口**：`docs/V1-验收检查表.md` 把规格书 **109 条 FR** 逐条对照证据
  ...
  **待办与未完成项的唯一入口见 `docs/V1-收尾待办.md`**（含环境要点与踩坑记录）。
  ...
  v1.1.15 / v1.1.19 / v1.1.20**（见 GitHub Releases；**跳过 v1.1.6**——那次发布已撤回、其提交不在 `main`，
  以及 **1.1.11~1.1.14、1.1.16~1.1.18**——同一 merge 线上的内部验证构建、从未发布，故由 v1.1.10 跳到 v1.1.15、
  v1.1.15 跳到 v1.1.19）。
