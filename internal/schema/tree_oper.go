	...
	root := K("", "root",
		K("show", "显示系统状态与配置",
			...
			K("system", "系统信息",
				...
				K("kernel", "内核启动基线（cmdline / 运行实际 / 配置期望 三方对照，FR-SYS-014）"),
				K("hardware", "硬件健康：温度/风扇/电源/SMART（FR-SYS-012）"),
				K("core-dumps", "崩溃转储清单（VPP/QEMU/nfvisd）"),
				K("tech-support", "诊断归档清单"),
				K("configuration", "配置查看",
					K("sessions", "candidate 持锁会话列表（FR-CFG-009）"),
			...
			K("bonds", "链路聚合列表（FR-NET-017）",
				P("<name>", "bond 名", "",
					K("detail", "成员口各自 link/LACP actor-partner 信息"),
				),
			),
			K("lldp", "LLDP（FR-NET-018）",
				K("neighbors", "邻居表",
					Opt(K("interface", "按接口过滤", P("<ifname>", "接口名", DynIfnames))),
				),
			),
			...
			K("resource-pools", "资源池：总量/已分配/空闲（含 vpp-reserved，FR-SYS-010）"),
			K("alarms", "告警列表",
			...
			K("users", "本地用户与 class"),
			K("configuration", "配置显示（FR-CFG-006）",
				K("candidate", "当前持锁会话的 candidate"),
		...
		Op(K("request", "运维动作",
			K("virtual-machine-functions", "VM VNF 操作（FR-CMP-011）",
				P("<name>", "VNF 名", DynVMs,
					K("start", "启动"),
					K("stop", "停止（ACPI 关机，超时强杀）"),
					K("restart", "重启"),
					K("console", "进入串口（Ctrl-] 退出，FR-CMP-014）"),
					K("snapshot", "快照操作（FR-CMP-015）",
					...
					Su(K("delete", "删除 VNF（级联 vNIC/VPP 端口/快照，FR-CMP-013）")),
				),
			),
			K("container-functions", "容器 VNF 操作（FR-CMP-021）",
				P("<name>", "容器名", DynContainers,
					K("start", "启动"),
					K("stop", "停止"),
					K("restart", "重启"),
			...
			K("interfaces", "接口启停与驱动接管（FR-NET-003/FR-NET-001）",
				P("<ifname>", "接口名", DynIfnames,
					K("enable", "启用"),
					K("disable", "禁用"),
			...
			K("sriov", "SR-IOV VF 管理（FR-NET-004）",
				K("create-vfs", "在物理口上创建 VF",
					P("<ifname>", "物理口", DynIfnames),
			...
			Su(K("vpp", "VPP 数据面操作",
				K("restart", "按 committed 配置重建数据面并恢复收敛（FR-SYS-009）"),
				K("trace", "数据面抓包（FR-OPS-042）",
					K("start", "开始抓包",
						K("interface", "抓包接口", P("<ifname>", "接口名", DynIfnames)),
			...
			K("system", "系统操作",
				...
				K("password", "口令",
					K("change", "登录者自助改密（验证旧口令，FR-SEC-008）"),
				),
				Su(K("storage", "存储管理",
		...
		Su(K("configure", "进入配置模式（§4：仅 super-user）")),
		Op(K("ping", "连通性测试",
			PT("<host>", "string", "目标地址"),
		...
		Su(K("clear", "清除",
			K("interfaces", "接口",
				K("statistics", "清零统计计数", Opt(P("<ifname>", "接口名", DynIfnames))),
			),
		)),
		Su(K("start", "启动",
